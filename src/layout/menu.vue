  ...
  <div class="menu-wrap">
    <ay-menu :defaultActive="defaultActive" @activeIndex="handleRouteChange">
      <ay-menu-item-group title="推荐">
        <ay-menu-item index="Recommendation" id="Recommendation">
          发现音乐
        </ay-menu-item>
...
export default {
  ...
  mounted() {
    console.log("我是this.$route.name", this.$route.name);
    this.$bus.$on("UpdateMenu", () => {
      let event = new Event("click", { bubbles: true, cancelable: true });
      document.getElementById("Recommendation").dispatchEvent(event);
    });
  },
};
</script>
